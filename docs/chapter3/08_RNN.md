...
回到最初的例子，看看 RNN 是如何解决“**播放周杰伦的《稻香》**”的分类问题的。首先需要**准备输入**，将句子**分词**为 `["播放", "周杰伦", "的", "《稻香》"]`，并转换为 ID 序列 `[23, 58, 102, 203]`（假设），再通过**词嵌入**得到 4 个词向量 $x_1, x_2, x_3, x_4$。假设每个向量是 128 维，则输入序列的形状为 $(T, E) = (4, 128)$。在此基础上，RNN 会按**顺序逐步处理**这个向量序列，并不断更新其内部的“记忆”（隐藏状态 $h$），这里假设隐藏状态的维度为 64（$H=64$），且在处理第一个词之前，初始隐藏状态 $h_0$ 通常被初始化为零向量。
